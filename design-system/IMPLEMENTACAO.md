# Implementação — Fotografia na HOF Design System

Documentação técnica de **como cada item é implementado**, para subir em qualquer infra (HTML/CSS puro, qualquer stack via tokens, ou Framer). Tudo aqui reflete o que está no showcase publicado (`index.html`) e no pacote `design-system/`. Em caso de divergência, **o showcase + `fotografia-na-hof-design-tokens.json` são a fonte da verdade**; o `fotografia-na-hof-design-system.css` (drop-in) espelha esses tokens.
...
```
repo (fotografia-na-hof-design-system)
...
└── design-system/                   # PACOTE consumível
    ...
    ├── fotografia-na-hof-design-system.css     # DROP-IN: tokens + reset + foco + motion + tipografia + botões + chips/status
    ├── fotografia-na-hof-design-tokens.json    # Tokens legíveis por máquina (Framer, Style Dictionary-like, IA)
    ...
    └── copy-deck.fotografia-na-hof.json        # Strings de produto do showcase
```

**Duas camadas de classe (importante):**
- **Pacote drop-in** usa classes prefixadas **`fnh-*`** (`.fnh-btn`, `.fnh-fill`, `.fnh-h1`, `.fnh-status`…). É o que você consome num projeto.
...
3. **Escolha do usuário** persistida em `localStorage['fnh-theme']` (`'light'`|`'dark'`), que vence o sistema.
...
```js
var t = localStorage.getItem('fnh-theme');
if (t !== 'light' && t !== 'dark')
...
Cores institucionais da marca: violeta `#7A1AD6`, lavanda `#C286FF` e azul profundo `#204A8A` (do gradiente do logo) mais o apoio herdado. Nada deve sair daqui.

| Token | Hex | Papel |
|---|---|---|
| `--brand-roxo` | `#7A1AD6` | predominante |
| `--brand-lilas` | `#C286FF` | accent claro |
...
|---|---|---|---|
| `--bg` | `#0A0A14` | `#FAFAFA` | fundo da página |
| `--bg2` | `#121022` | `#F4F2FC` | fundo alt / app shell |
| `--card` | `#1A1732` | `#F9F8FE` | superfície de cartão/controle |
| `--card2` | `#221E40` | `#EEECFA` | superfície elevada/hover |
| `--line` | `rgba(194,134,255,.14)` | `rgba(122,26,214,.16)` | bordas/divisores |
| `--txt` | `#F8F8FD` | `#1A1533` | texto principal |
| `--mut` | `#C4BFD9` | `#5F5A82` | texto secundário |
| `--legal-mut` | `#8880B5` | `#5F5A82` | texto legal/rodapé |
| `--roxo` | `#341060` | `#341060` | roxo profundo (gradiente) |
| `--roxo2` | `#7A1AD6` | `#7A1AD6` | primária (seleção, dia/linha selecionada) |
| `--roxo-bright` | `#8B33E3` | `#8B33E3` | roxo claro (base do `--cta` escuro) |

#### CTA — token theme-aware (acessibilidade de contraste de componente)

O **CTA** (botão preenchido/sólido) é o único token de cor que muda de tom entre os temas **por acessibilidade de componente** (WCAG 1.4.11 — *Non-text Contrast*, ≥3:1 do botão vs. fundo). No tema escuro, o violeta base `#7A1AD6` não é usado como CTA (daria ~2.8:1 contra o fundo e reprovaria); o CTA nasce com o brilhante `#8B33E3`.

| Token | Dark | Light | Uso |
|---|---|---|---|
| `--cta-grad` | `linear-gradient(120deg,#8B33E3,#2E5FA8)` | `linear-gradient(120deg,#7A1AD6,#204A8A)` | fundo do botão preenchido (`.b.fill`/`.fnh-fill`) |
| `--cta-solid` | `#8B33E3` | `#7A1AD6` | fundo do botão sólido (`.b.solid`/`.fnh-solid`) |
| `--cta-solid-h` | `#2E5FA8` | `#8B33E3` | hover do sólido |
...
| `--lilas-soft` | `#D4B3FF` | `#6929B8` | accent hover |
| `--logo` | `#FFFFFF` | `#7A1AD6` | cor do lockup SVG |
...
| `--info` / `--info-bg` | `#C286FF` / `rgba(194,134,255,.14)` | `#5E4A8C` / `rgba(94,74,140,.10)` |

> `--danger` light = `#BE2C45` (aprofundado para passar AA 4.5:1 no badge de erro; valor de nascença da marca).
...
/* Code (escuro nos 2 temas) */ --code-bg:#060409  --code-txt:#D4B3FF  --code-comment:#9A8CC0  --code-key:#FFE4A4
...
--sh / --sh-strong: rgba(122,26,214,.40/.50) dark · .18/.26 light  (glow dos botões fill)
```
**Regra:** use o papel (`--elev-raised/overlay/modal`), não o número.

### 2.5 Formato do `fotografia-na-hof-design-tokens.json`
...
Classe base **`.b`** (drop-in: `.fnh-btn`). Composição: `.b` + tamanho (`.sm`/`.md`/`.lg`) + variante (`.fill`/`.solid`/`.outline`/`.ghost`/`.gold`/`.gold-o`). Ícone interno: `.ico` (drop-in `.fnh-ico`).

> **CTA (fill/solid) lê `--cta-*`, não `--roxo2`/`--roxo-bright` direto.** `.b.fill`/`.fnh-fill` usa `--cta-grad` + `--cta-ink`; `.b.solid`/`.fnh-solid` usa `--cta-solid` (hover `--cta-solid-h`). O `--cta` é **theme-aware**: no escuro é roxo mais claro (`#8B33E3`) para o botão passar contraste de componente (≥3:1 vs. fundo, WCAG 1.4.11); no claro fica `#7A1AD6` (inalterado). Ver "CTA — token theme-aware" na seção 2.3.
...
```css
.fnh-chip{background:var(--card);border:1px solid var(--line);color:var(--txt);
         padding:9px 16px;border-radius:30px}                 /* neutro, cor via token */
.fnh-badge{color:var(--gold-ink);border:1px solid var(--gold-line);font-size:10.5px;
          padding:4px 10px;border-radius:20px}                /* destaque dourado */
.fnh-status{padding:9px 15px;border-radius:30px;border:1px solid currentColor}
.fnh-status.is-success{color:var(--success);background:var(--success-bg)}
.fnh-status.is-warning{color:var(--warning);background:var(--warning-bg)}
.fnh-status.is-danger {color:var(--danger);background:var(--danger-bg)}
.fnh-status.is-info   {color:var(--info);background:var(--info-bg)}
```
...
### 5.4 Sobreposições
```css
.modal-scrim{position:absolute;inset:0;background:radial-gradient(... rgba(194,134,255,.30) ...),rgba(52,16,96,.55);
...
```
Scrim usa `backdrop-filter:blur(6px)` + matiz roxo (não cinza). Aparição/posicionamento de tip/pop é via interação (hover/JS) com `transition:.2s var(--ease)`.
...
- **Tema:** init anti-flash + toggle `#themeToggle` (ver 2.1). Chave `localStorage['fnh-theme']`.
...
- **Contraste WCAG 2.1 AA em 2 níveis**, medido nos 2 temas:
  ...
  - O **CTA do tema escuro usa o violeta brilhante** `#8B33E3` exatamente para passar o **nível 2** (o base `#7A1AD6` daria ~2.8:1 contra o fundo escuro e reprovaria 1.4.11). No tema claro o CTA usa `#7A1AD6` com fim de gradiente no azul `#204A8A`.
...
<html lang="pt-BR"><head>
  ...
  <button class="fnh-btn fnh-fill">Assinar agora</button>
</body></html>
```
...
**B) Tokens (qualquer stack)** — importe `fotografia-na-hof-design-tokens.json` e gere CSS vars / JS / Tailwind theme. Componentes consomem token, nunca hex.
...
- **Repos:** `Eddie-FacialAcademy/fotografia-na-hof-design-system` (irmão: `corporal-class-design-system`).
...
- **URL ao vivo:** https://eddie-facialacademy.github.io/fotografia-na-hof-design-system/
...
- **Arquitetura de tokens:** `primitive → semantic/intent → component`; o semântico **nunca sugere valor** (`--btn-bg`, não `--roxo-500`); componente referencia o semântico.
...
## 13. Diferenças por marca (Fotografia na HOF × Corporal)

Mesma arquitetura, JS, componentes, escalas e semânticas. Mudam:

| Aspecto | Fotografia na HOF | Corporal Class |
|---|---|---|
| Arquivos | `fotografia-na-hof-design-system.css` · `fotografia-na-hof-design-tokens.json` · `copy-deck.fotografia-na-hof.json` | `corporal-design-system.css` · `corporal-design-tokens.json` · `copy-deck.corporal.json` |
| Prefixo de classe (drop-in) | `fnh-*` | `cc-*` |
| Chave de tema | `localStorage['fnh-theme']` | `localStorage['cc-theme']` |
| id do cmdk | `cmdk-list-fnh` | `cmdk-list-co` |
| Marca / mãe | Fotografia na HOF / Facial Academy | Corporal Class / Corporal Academy |
| Token primário | `--roxo2 #7A1AD6` | `--bordo2 #D6515C` |
| Accent interativo | `--lilas` (#C286FF dark / #7A1AD6 light) | `--coral` (#E88A92 dark / #C2434E light) |
| `--info` (dark/light) | `#C286FF` / `#5E4A8C` (roxo) | `#74C0D8` / `#2A7286` (**teal**, p/ não confundir com o coral/vermelho) |
| Foco (`--focus-ring` dark/light) | `#C286FF` / `#7A1AD6` | `#E88A92` / `#C2434E` |
| Sombra (matiz) | `rgba(122,26,214,…)` | `rgba(214,81,92,…)` |
...
> Trocar de marca = trocar a linha de import (`fotografia-na-hof-…`↔`corporal-…`) e o prefixo de classe. O resto do código é idêntico.
